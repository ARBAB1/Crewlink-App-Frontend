  ...
  const fetchGroupDetails = async () => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/messages/get-group-details/${route?.params?.group_id}`,
        {
          method: 'GET',
          headers: {
        ...
        <>
          <View style={styles.messageWrapper}>
            {items?.item?.senderUserId == user_id ? (
              <Pressable
                onLongPress={() => OpenMessageAction(items?.item)}
                style={
                  isSelected
                    ? styles.messageContainer2Selected
                ...
                onPress={() =>
                  isOpenAction
                    ...
                    ? navigation.navigate('PostDetail', items?.item?.post_id)
                    : null
                }>
                  ...
                  <>
                    <Pressable
                      onPress={() =>
                        MediaDetail(
                          items?.item,
                          isVideo[1] == '4' ? false : true,
                        )
                      }
                      onLongPress={() => GetReply(items?.item)}>
                        ...
                        {isVideo[1] == '4' ? (
                          ...
                          <>
                            ...
                            {items?.item?.message !== '' ? (
                              ...
                              <View style={styles.BottomInfoBar}>
                                <Text style={styles.TimeAgoWhite}>
                                  {formattedTime}
                                </Text>
                                {items?.item?.isSend == false ? (
                                  <AntDesign
                        ...
                        <View
                          ...
                          <View
                            ...
                            <View
                              ...
                              <Text style={styles.ReplyMsgUserName}>
                                Replying
                              </Text>
                              <ActivityIndicator
                                size={'small'}
                                color={global.white}
                                style={{marginLeft: ResponsiveSize(10)}}
                              />
                            </View>
                          </View>
                        </View>
                          ...
                          {items?.item?.repliedMessage != null && (
                            <View
                              ...
                              {items?.item?.repliedMessage?.is_media == 'Y' ? (
                                <View
                                  style={{
                                    ...styles.ReplyMsgBoxMine,
                                    flexDirection: 'row',
                                    alignItems: 'center',
                                    justifyContent: 'space-between',
                                  }}>
                                  {items?.item?.repliedMessage?.senderUserId ==
                                  user_id ? (
                                    <View>
                                      <Text style={styles.ReplyMsgUserName}>
                                        You
                                  ...
                                  <FastImage
                                    source={{
                                      uri: items?.item?.repliedMessageMedia
                                        ?.media_url,
                                      priority: FastImage.priority.high,
                                    }}
                                    style={{
                                ...
                                <View style={styles.ReplyMsgBoxMine}>
                                  ...
                                  <Text
                                    ...
                                    {items?.item?.repliedMessage?.message}
                                  </Text>
                                </View>
                              )}
                            </View>
              ...
              <Pressable
                onLongPress={() => GetReply(items?.item)}
                style={styles.messageContainer1}>
                {items?.item?.is_media == 'Y' ? (
                  <>
                    <ImageBackground
                      source={
...
//   };
  ...
  const DeleteMessage = async () => {
    ...
    socket
      .on('connect')
      .emit('deleteGroupMessage', {
        messageId: multiSelected,
      })
      .emit('oldGroupMessages', {group_id: groupDetails?.group_id})
      .on('groupMessages', data => {
        CancelReply();
        setRecentChats(data?.message);
            ...
            <View style={styles.logoSideChatAction}>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                ...
                <TouchableOpacity
                  ...
                  style={{marginHorizontal: ResponsiveSize(10)}}>
                  {deleteMessageLoad ? (
                    <ActivityIndicator
                      size={'small'}
                      color={global.primaryColor}
            ...
            <Animated.View style={{...styles.ReplyBox, opacity: fadeAnim}}>
              <View style={styles.ReplyInfo}>
                {ReplyMessage?.senderUserId == user_id ? (
                  ...
                  <TextC
                    text={`Replying to ${groupDetails?.group_name}`}
                    font={'Montserrat-Medium'}
                    size={ResponsiveSize(10)}
                    style={{
                      color: global.description,
